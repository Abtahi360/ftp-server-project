    ...
    <div class="card">
        ...
        <form method="post" action="../../controllers/moderatorController.php?action=upload_content"
            ...
            <div class="form-group">
                ...
                <small style="color:var(--text3);font-size:12px;display:block;margin-top:5px">Allowed: MP4, AVI, MKV, MP3, WAV, ZIP, PDF, DOC, JPG, PNG — max 100 MB</small>
                <span class="field-error" id="upFileErr"></span>
            </div>
            <div style="display:flex;gap:10px;margin-top:8px">
                <button type="submit" name="submit" class="btn btn-primary">⬆️ Upload</button>
...
function validateModUpload() {
    var ok=true; var t=document.getElementById('upTitle').value.trim(); var c=document.getElementById('upCat').value; var f=document.getElementById('upFile');
    clearErrors(['upTitleErr','upCatErr','upFileErr']);
    if(!t){showError('upTitleErr','Title required.');ok=false;}
    if(!c||c==='0'){showError('upCatErr','Select a category.');ok=false;}
    if(f.files.length===0){showError('upFileErr','Select a file.');ok=false;}
    else{var allowed=['mp4','avi','mkv','mp3','wav','zip','pdf','doc','docx','jpg','jpeg','png','gif'];var ext=f.files[0].name.split('.').pop().toLowerCase();if(allowed.indexOf(ext)===-1){showError('upFileErr','Type not allowed.');ok=false;}else if(f.files[0].size>100*1024*1024){showError('upFileErr','Max 100 MB.');ok=false;}}
    return ok;
}
</script>
