...
<div id="ajaxMsg" style="display:none;margin-bottom:14px;"></div>

<?php if (empty($requests)): ?>
    <div class="empty-state">
        <div class="empty-icon">📭</div>
        <h3>No requests yet</h3>
        <p>Members' content requests will appear here.</p>
    </div>
<?php else: ?>
    <div class="table-wrapper">
        <div class="table-scroll">
            <table class="data-table">
                <thead>
                    <tr><th>#</th><th>Content Requested</th><th>Category</th><th>Message</th><th>Status</th><th>Date</th><th>Action</th></tr>
                </thead>
                <tbody>
                            ...
                            <td>
                                <?php if ($req['status'] === 'pending'): ?>
                                    <div class="action-cell">
                                        <button onclick="updateStatus(<?= intval($req['id']) ?>,'fulfilled')" class="btn btn-success btn-sm">✅ Fulfill</button>
                                        <button onclick="updateStatus(<?= intval($req['id']) ?>,'rejected')"  class="btn btn-danger btn-sm">❌ Reject</button>
                                    </div>
                                <?php else: ?>
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (resp.success) {
                ...
                document.getElementById('row-' + id).cells[document.getElementById('row-' + id).cells.length - 1].innerHTML = '<span class="muted">Done</span>';
                msg.className = 'alert alert-success'; msg.innerHTML = '✅ ' + resp.message;
            } else {
                msg.className = 'alert alert-error'; msg.innerHTML = '⚠️ ' + (resp.error || 'Update failed.');
            }
